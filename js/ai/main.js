...
function initIntroSectionAnimation() {
    ...
    const tl = gsap.timeline({
        ...
        scrollTrigger: {
            ...
            id: 'intro-animation',
            // 역스크롤링 시 충돌 방지를 위한 설정
            toggleActions: 'play none none reverse',
            onEnter: () => {
                // 애니메이션 시작 시 상태 초기화
                gsap.set([title1, title2, desc1, desc2], { opacity: 0, yPercent: 100 });
                gsap.set(countUp, { opacity: 0, yPercent: 50 });
            },
            onEnterBack: () => {
                // 역스크롤링 시 상태 복원
                gsap.set([title1, title2, desc1, desc2], { opacity: 1, yPercent: 0 });
                gsap.set(countUp, { opacity: 1, yPercent: 0 });
            }
        }
    });
    ...
    if (title2) {
        tl.fromTo(title2, { opacity: 0, yPercent: 100 }, { opacity: 1, yPercent: 0, duration: 0.4 }, '-=0.2');
    }
    if (desc1) {
        tl.fromTo(desc1, { opacity: 0, yPercent: 100 }, { opacity: 1, yPercent: 0, duration: 0.4 });
    }
    if (desc2) {
        tl.fromTo(desc2, { opacity: 0, yPercent: 100 }, { opacity: 1, yPercent: 0, duration: 0.4 }, '-=0.2');
    }
    ...
    if (isMobile) {
        // 모바일: 개별 애니메이션
        if (countUp[0]) {
            tl.fromTo(countUp[0], { opacity: 0, yPercent: 50 }, {
                opacity: 1,
                yPercent: 0,
                duration: 0.4,
                onStart: () => {
        ...
        if (countUp[1]) {
            tl.fromTo(countUp[1], { opacity: 0, yPercent: 50 }, {
                opacity: 1,
                yPercent: 0,
                duration: 0.4,
                onStart: () => {
        ...
        // 데스크톱: 그룹 애니메이션
        if (countUp.length > 0) {
            tl.fromTo(countUp, { opacity: 0, yPercent: 50 }, {
                opacity: 1,
                yPercent: 0,
                duration: 0.4,
                onComplete: () => {
